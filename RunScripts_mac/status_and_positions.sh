...
monitor_accounts() {
    ...
    for accountId in "${ACCOUNT_SET[@]}"; do
        ...
        if [[ "$status" == "STALLED" ]]; then
            echo "PANIC : $accountId" 

            ts=$(date +%Y%m%d_%H%M%S)
            python3 "$SNAPSHOT_PY" --private_key "$(python3 "$DECRYPT" "$PASSWORD" "$KEY_PREFIX" "$(echo "$accountId" | grep -o '[0-9]*')" )" --account_id "$accountId" > "$SNAP_DIR/$accountId.$ts.json"

            python3 "$CANCEL_PY" --private_key "$(python3 "$DECRYPT" "$PASSWORD" "$KEY_PREFIX" "$(echo "$accountId" | grep -o '[0-9]*')" )" --account_id "$accountId" > /dev/null
